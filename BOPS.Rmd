...
```
##Basically, what I need to explain is I tried to address the endo model and rho is insig, so I can conclude that the treatment model doesn't has been impact on the model. So simple probit regession will do the work. Ratio of pick up in store in 2012 and 2013. If I run in on only 2012 data, I will see similar result like Selina, no impact on return. 2013 is included to capture longer term effect of bops. Look at 5998 seperately, 2 and 6 are in the USA. Online sale represent 7% of total sale, Bops trigger brick and mortal sale. This stragety trigger sale on phsiacally sales. We do not have additional data, to show the positive side of this implementation.
...
<!-- library("MASS") -->
<!-- model<-glm(return~bops+price+age_band+est_income_code+male+homeowner+kid+store2+store5998+JAN+FEB+MAR+APR+MAY+JUN+JUL+AUG+SEP+OCT+NOV+bridal+goldwedbands+solitaires+dfashion+semiprecious+mens+goldearrings+specialevent+beads+piercings+dsoljewelry+gold+watch+preowned+specialized+events+diawedband, family=binomial(link="probit")) -->
...
<!-- # install.packages("usdm") -->
<!-- # library(usdm) -->
<!-- # df=data.frame(bops,price,age_band,est_income_code,male,homeowner,kid,store2,store5998,JAN,FEB,MAR,APR,MAY,JUN,JUL,AUG,SEP,OCT,NOV,bridal,goldwedbands,solitaires,dfashion,semiprecious,mens,goldearrings,specialevent,beads,piercings,dsoljewelry,gold,watch,preowned,specialized,events,diawedband) -->

<!-- # Generates the correlation matrix, store 6 is excluded becausecor(store6,store2)=0.8, cor(store2,store5998)=0.55    -->
<!-- #The rest all less than 0.8 so its ok. -->
<!-- cor(df)  -->

<!-- vif(df) # Calculates VIF scores # all between range (1, 1.66) Less than 3, so it is ok -->
<!-- ``` -->
...
<!-- install.packages("mfx") -->
<!-- library(mfx) -->
<!-- model<-glm(return~bops+price+age_band+est_income_code+factor(gender)+factor(month)+factor(store_number)+factor(summary), family=binomial(link="probit")) -->
<!-- summary(model) -->
<!-- wald.test(b=coef(model),Sigma=vcov(model),Terms=19:20) #test the overall effect of store number, significant means the overall effect of mall is good -->
<!-- wald.test(b=coef(model),Sigma=vcov(model),Terms=21:37)# test the overall effect of catagories, significant means the overall effect of mall is good -->

<!-- with(model, null.deviance - deviance) # if it is significant, the model fits data. -->
<!-- with(model, df.null - df.residual) -->
<!-- with(model, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE)) #The output produced by summary(probit1) included indices of fit (shown below the coefficients), including the null and deviance residuals and the AIC. To find the difference in deviance for the two models (i.e., the test statistic) we can use the with command.The chi-square of 41.56 with 5 degrees of freedom and an associated p-value of less than 0.001 tells us that our model as a whole fits significantly better than an empty model. -->